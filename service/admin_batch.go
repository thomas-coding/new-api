...
func ExecuteBatchRedemptions(ctx BatchOperationContext, req dto.BatchRedemptionsExecuteRequest) (*dto.BatchExecuteResponse, error) {
	...
	for i := 0; i < validated.Count; i++ {
		...
		redemption := &model.Redemption{
			...
			RedeemType:         model.RedemptionTypeSubscription,
			Quota:              0,
		...
		successCount++
		lines = append(lines, formatRedemptionExportLine(validated.ExportFormat, key, plan.Title, validated.ExpiredTime, batchID))
...
func formatRedemptionExportLine(format string, code string, planTitle string, expiredTime int64, batchID string) string {
	if format == batchExportFormatCSV {
		return fmt.Sprintf("%s,%s,%d,%s", code, planTitle, expiredTime, batchID)
	}
	return code
}
