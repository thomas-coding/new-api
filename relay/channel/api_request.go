...
const arrouteAffinityHeader = "X-Arroute-Affinity-Key"
...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	"x-api-key":      {},
	"x-goog-api-key": {},
	// Internal routing hint; do not let clients inject it via wildcard passthrough.
	"x-arroute-affinity-key": {},
...
func applyArrouteAffinityHeader(headers http.Header, info *common.RelayInfo, requestURL string) {
	if headers == nil || info == nil || info.UserId <= 0 {
		return
	}
	if strings.TrimSpace(headers.Get(arrouteAffinityHeader)) != "" {
		return
	}
	if !isArrouteCliproxyTarget(requestURL, info.ChannelBaseUrl) {
		return
	}
	headers.Set(arrouteAffinityHeader, fmt.Sprintf("user:%d", info.UserId))
}

func isArrouteCliproxyTarget(urls ...string) bool {
...
func DoApiRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	applyArrouteAffinityHeader(req.Header, info, fullRequestURL)
...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	applyArrouteAffinityHeader(targetHeader, info, fullRequestURL)
	// 在 SetupRequestHeader 之后应用 Header Override，确保用户设置优先级最高
	// 这样可以覆盖默认的 Authorization header 设置
	headerOverride, err := processHeaderOverride(info, c)
	if err != nil {
